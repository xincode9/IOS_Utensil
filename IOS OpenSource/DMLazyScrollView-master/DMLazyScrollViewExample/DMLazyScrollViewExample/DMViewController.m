...
- (void)viewDidLoad {
    ...
    // PREPARE LAZY VIEW
    CGRect rect = CGRectMake(20, 100, self.view.frame.size.width-40, self.view.frame.size.height-50-100);
    ...
    lazyScrollView.dataSource = ^(NSUInteger index) {
        return [strongSelf controllerAtIndex:index];
    };
    ...
    _label= [[UILabel alloc] initWithFrame:CGRectMake(0, 50, 100, 100)];
...
{
    _label.text = [NSString stringWithFormat:@"%d",currentPageIndex];
}
...
{
   _label.text = [NSString stringWithFormat:@"%d",pageIndex];
}
